...
import { PesticideForm } from '@/lib/api/types';

export default function PesticideFormDetailPage() {
    const router = useRouter();
    const params = useParams();
    const formId = params.id as string;

    const [form, setForm] = useState<PesticideForm | null>(null);
    ...
    useEffect(() => {
        const fetchForm = async () => {
            try {
                const data = await formsClient.getPesticideForm(formId);
                setForm(data);
            } catch (err) {
                    ...
                    {form && (
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-zinc-900 dark:text-zinc-50">{form.pesticide_name}</p>
                            </div>
